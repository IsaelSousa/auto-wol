...
/**
 * Um dispositivo "rastreado" e o que o usuario realmente usa: adicionado
 * manualmente, fixado, ja acordado pelo menos uma vez, ou a propria maquina.
 * O resto e so ruido de varredura.
 */
export function isTracked(d) {
  return Boolean(d.manual || d.self || d.pinned || (d.wakeCount || 0) > 0);
}

/**
 * Remove da persistencia os dispositivos que sao apenas ruido de varredura
 * (descobertos automaticamente e nunca usados). Retorna quantos foram removidos.
 */
export async function pruneDiscovered() {
  const devices = await readAll();
  const keep = devices.filter(isTracked);
  await writeAll(keep);
  return devices.length - keep.length;
}

/**
 * Registra que um dispositivo foi acordado: incrementa o contador de uso
 * (wakeCount) e guarda o horario (lastWokenAt). E com base nesses campos que a
 * interface destaca automaticamente os dispositivos "mais usados / recentes".
 */
export async function recordWake(id) {
  const devices = await readAll();
  const idx = devices.findIndex((d) => d.id === id);
  if (idx < 0) return null;
  const device = devices[idx];
  device.wakeCount = (device.wakeCount || 0) + 1;
  device.lastWokenAt = new Date().toISOString();
  await writeAll(devices);
  return device;
}
